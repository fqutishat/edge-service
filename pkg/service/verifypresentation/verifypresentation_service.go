...
func (s *Service) checkCredentialStrict(lazy []*LazyCredential) error { //nolint:gocognit
	for _, input := range lazy {
		...
		displayCredentialMap, err := cred.CreateDisplayCredentialMap(verifiable.DisplayAllDisclosures())
		if err != nil {
			return fmt.Errorf("error %s", cred.SDJWTHashAlg)
		}

		if err := jsonld.ValidateJSONLDMap(displayCredentialMap,
